...
describe('Testing Array.prototype.select parameters', () => {
  ...
  it('it must return last 5 element from array with numbers when first parametr is -5', () => {
    const expectedValue: Array<number> = [5, 6, 7, 8, 9];
  ...
  it('it must return last element from array with strings when first parametr is -1', () => {
    const arrWithStrings: Array<string> = ['erq', 'kosik', 'refix'];
    const expectedValue: Array<string> = ['refix'];
    const output: Array<string> = arrWithStrings.select(-1);
